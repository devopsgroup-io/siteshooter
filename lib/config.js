...
config.getSettings = function(options) {
    ...
    // check for specified file and update working directory
    if(self.options.commands.file){
        self.process.set('working_directory', path.resolve(self.options.commands.file));
    }
    else{
...
config.isQuiet= function() {
    return this.options.commands.quiet;
};
