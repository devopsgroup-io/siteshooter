...
PDFShooter.prototype.start = function() {
    ...
    if (domainName.indexOf('visual.force') > -1 ){
        ...
        self.options.pages = self.options.pages.map(function(page){

            return new urlParse(page, true);
        });
    }
        ...
        return {
            ...
            'files': self.options.pages.map(function(page){
                ...
                if( typeof page === 'object' ){
                    ...
                    fileName = path.join(outputPath, utils.urlToDir(domainName + '/' + page.query.name + page.hash), item.width + '.png');
                    if(page.query.pevent){
                        fileName = path.join(outputPath, utils.urlToDir(domainName + '/' + page.query.name + page.hash), item.width + '-event-' + page.query.pevent + '.png');
                    }
                }
                else{
                    fileName = path.join(outputPath, utils.urlToDir(page), item.width + '.png');
                }
                return fileName;
            })
        };
