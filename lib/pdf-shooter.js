...
PDFShooter.prototype.start = function() {
    ...
    pdfShooter.options.pages = pdfShooter.options.pages.filter(function(page) {
        return !page.match(regExclusions);
    });

    if (domainName.indexOf('visual.force') > -1 ){

        var folderName, urlParse = require('url-parse');

        domainName = new urlParse(domainName, true).host;

        //outputPath = path.join(outputPath, domainName);

        pdfShooter.options.pages = pdfShooter.options.pages.map(function(page){

            folderName = new urlParse(page, true);
            return folderName.query.name + folderName.hash;
        });
    ...
    pdfs = this.options.viewports.map(function(item, i) {
        return {
            ...
            'files': pdfShooter.options.pages.map(function(page){
                return path.join(outputPath, domainName, utils.urlToDir(page), item.width + '.png');
            })
        };
    });

    pdfShooter.website.get().then(function() {
