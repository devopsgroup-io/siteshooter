        ...
        if (err) {
            ...
            request(url, function(err, response) {
                ...
                if (response.statusCode === 200) {
                    ...
                    utils.log.note('  ⤷ Authentication required for: ' + url);
                    ...
                    if (options.domain.auth.user !== null && options.domain.auth.pwd !== null) {
                        ...
                        utils.log.note('    ✔︎ Credentials are set');
                        ...
                        request.get(url, {
                            ...
                            if (response.statusCode === 301) {
                                ...
                                var redirctLocation = response.headers.location;

                                utils.log.note('    ⤷ redirecting to ' + redirctLocation);

                                request.get(redirctLocation, {
                                    'followRedirect': false,
                                    'auth': {
                                        'user': options.domain.auth.user,
                                        'pass': options.domain.auth.pwd,
