...
var renderUrlsToFile = function(ph, options, urls, callbackPerUrl, callbackFinal) {
    ...
    getFileName = function(folder) {
        ...
        var fileName = options.viewports[key].width + '.png';
        ...
        if (folder.indexOf('visual.force') > -1) {
            ...
            if ( folderName.query.pevent ){
                fileName = options.viewports[key].width + '-event-' + folderName.query.pevent + '.png';
            }
        }
        return path.join(options.paths.output, folder, fileName);
    };
