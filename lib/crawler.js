...
var Crawler = function(options) {
    ...
    this.regExclusions = new RegExp('\.(' + exclusions + ')', 'i');
...
Crawler.prototype.crawlHost = function() {
    ...
    if ( this.options.uri.host.indexOf('visual.force') > -1 ) {
        ...
        this.crawler.on('fetchcomplete', function(item, data, res) {
            ...
            if(!item.path.match(self.regExclusions)){
            ...
            sitemap.writeXML(this.sitemapLinks, function(err) {
                ...
                if (err) {
                    ...
                    return utils.executeWhen(error.collector404.length > 0, function() {
                        return utils.writeFile('.siteshooter/urls_404.json', JSON.stringify(error.collector404));
                    })
