...
Crawler.prototype.crawlHost = function() {
    ...
    utils.log.log(chalk.yellow.bold(' ⤷ Crawling Host: '), chalk.underline.yellow(this.uri.host));

    var deferred = Q.defer(),
        error = [],
        self = this;

    if ( this.uri.host.indexOf('visual.force') > -1 ) {

        utils.log.log(chalk.red.bold('\n   Salesforce Detected: --sitemap not supported.\n'));
        ...
        this.crawler.on('fetchcomplete', function(item, data, res) {
            ...
            utils.log.log(chalk.green.bold('   ✔︎ 200 '), chalk.green(item.url));
            ...
            if (item.stateData.contentType.replace(' ', '').toLowerCase() === 'text/html;charset=utf-8') {
                ...
                var meta = self.website.metadata($);
        ...
        this.crawler.on('fetchdisallowed', function(item) {
            if (!this.options.silent) {
                utils.log.log(chalk.red.bold('   ✗ 200 '), chalk.red(item.url, ' - Disallowed in robots.txt'));
            }
        }.bind(this));


        this.crawler.on('fetch404', function(item) {
            utils.log.log(chalk.red.bold('   ✗ 404 '), chalk.red(item.url));

            error.collector404.push({'referrer':item.referrer, 'url':item.url});
        });

        this.crawler.on('fetcherror', function(item, response) {
            utils.log.log(chalk.red.bold('   ✗ ' + response.statusCode, response.statusMessage), ' ', chalk.red(item.url));

            if (response.statusCode === 401) {
                utils.log.log(chalk.red('    Authentication required - please update your ', chalk.bold('siteshooter.yml'), ' file to include Auth user & pwd.'));
            }
        });
        ...
        this.crawler.on('complete', function() {
            ...
            sitemap.writeXML(this.sitemapLinks, function(err, sitemapFile) {
                ...
                if (err) {
                    ...
                    utils.log.log(chalk.green.bold('\n ✔︎'), chalk.green(sitemapFile + ' successfully created.'));
                    utils.log.log(chalk.white(' Added %s URLs to sitemap.'), this.sitemapLinks.length);
                    ...
                    */
                    ...
                    if (error.collector404.length > 0) {
                        this.writeCollections('urls_404.json', error.collector404, function() {
                            utils.log.log(chalk.white(' Added %s URLs to 404 collection file.'), error.collector404.length);
                        });
                    }
                    ...
                    this.writeContentCollection(function(err) {
                        if (err) {
                            ...
                            utils.log.log(chalk.white(' Added %s pages to content-collection.json\n'), this.website.contentCollection.pages.length);
