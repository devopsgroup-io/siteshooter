...
function get404s() {
    ...
    return Q.promise(function(promiseResolve, promiseReject) {
        ...
        return fs.readFile(path.join(config.process.get('working_directory'), '.siteshooter', 'urls_404.json'), function(err, data) {
...
function writeCSV(fileName, arrFields, arrData) {
    ...
    var csv = json2csv.parse(arrData,{
        fields: arrFields,
    });

    fs.writeFile(path.join(config.process.get('working_directory'), fileName + date + '.csv'), csv, function(err) {
        if (err) {
            deferred.reject(err);
        } else {
            deferred.resolve();
...
Website.prototype.getContentCollection = function() {
    ...
    return Q.promise(function(promiseResolve, promiseReject) {
        ...
        return fs.readFile(path.join(config.process.get('working_directory'), '.siteshooter', 'content-collection.json'), function(err, data) {
