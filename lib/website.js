...
function get404s(ignoreENOENT) {
    ...
    fs.readFile('urls_404.json', function(err, data) {
...
Website.prototype.get = function(callback) {

    var self = this,
        contentCollection = false,
        returnError;

    fs.readFile(self.contentFile, function(err, data) {
        ...
        if (err) {
            ...
            if (err.code === 'ENOENT') {
                ...
                utils.log.log('');
                utils.log.log(utils.log.chalk.yellow.bold('     Warning: '), utils.log.chalk.yellow(self.contentFile, 'does not exist. Skipping website content collection.'));
                utils.log.log('');
            ...
            contentCollection = JSON.parse(data);
        }

        return callback(returnError, contentCollection);
    });
...
Website.prototype.summary = function() {
    ...
    self.get(function(error, data) {
        ...
        if (error) {
            deferred.reject(Error(utils.log.chalk.bold(self.contentFile, 'does not exist.')));
        } else {
            ...
            return get404s(true).then(function(data404) {
                ...
                data.pages.forEach(function(page, i) {
                    ...
                    page.body.images.forEach(function(image, j) {
                        ...
                        if (image.alt === '') {
                            ...
                            summary.problem.altTags.push({
                                image: image.src,
                                url: page.loc
                            });

                            summary.problem.report.push({
                                'URL': page.loc,
                                'Issue': 'Missing Alt Tag',
                                'SRC' : self.options.domain.uri.href.substr(0 , self.options.domain.uri.href.lastIndexOf('/')) + image.src
                            });
                        }
                ...
                json2csv({
                    data: summary.problem.report,
                    fields: ['URL', 'Issue', 'SRC']
                }, function(err, csv) {

                    fs.writeFile('siteshooter-report-issues-' + date + '.csv', csv, function(err) {
                        if (err) {
                            deferred.reject(err);
                        } else {
                            deferred.resolve();
