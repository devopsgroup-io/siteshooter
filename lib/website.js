...
Website.prototype.get = function() {
    ...
    getLocalFile(website.contentFile, function(err, data) {
        if (err) {
            ...
            if( err.errno === -2){
                ...
                utils.log.log('');
                utils.log.log(chalk.yellow.bold('     Warning: '), chalk.yellow(website.contentFile, 'does not exist. Skipping website content collection.'));
                utils.log.log('');
