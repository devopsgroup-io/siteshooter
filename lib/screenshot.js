    ...
    renderViewPort = function(viewportCounter, page, status, url) {
        ...
        if (!!viewportCounter) {
            ...
            waitFor(function() {
                ...
                collectWebsiteInformation(page, url)
        ...
        if (urls.length > 0) {
            ...
            ph.createPage().then(function(page) {
                ...
                page.on('onLoadFinished', function(pageStatus) {
                    ...
                    utils.log.verbose('Function: onLoadFinished ', pageStatus);
                ...
                page.open(url).then(function(pageStatus) {
                    utils.log.verbose('PhantomJS Page Open:', pageStatus);
                });
    ...
    phantom.create(phantomOptions).then(function(ph) {
        ...
        return globby([path.join(options.paths.output, '**', '*.png')])
            .then(function(paths) {
                ...
                utils.log.verbose('\n Optimizing screenshot images. \n');

                // optimize screenshot images
                return Q.promise(function(promiseResolve, promiseReject) {

                    var mapImages = paths.map(function(imagePath) {
                        return imagemin([imagePath], path.parse(imagePath).dir, { use: [imageminPngquant({ quality: '60-80' })] });
                    });
