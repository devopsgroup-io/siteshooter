...
var excludeExtensions = ['gif', 'jpg', 'jpeg', 'png', 'ico', 'bmp', 'ogg', 'webp',
    ...
    'rar', '7z', 'css', 'js', 'gzip', 'exe', 'svg', 'pptx', 'cdn-cgi', 'xmlrpc', 'wp-comments-post'
];
...
function Sitemap(options) {
    ...
    utils.log.verbose('Sitemap: constructor', options);
    ...
    this.sitecrawlerOptions = this.options.sitecrawler;

    // look for user defined crawler exclusions
    if (Array.isArray(this.sitecrawlerOptions.exclude) && this.sitecrawlerOptions.exclude.length > 0) {
        excludeExtensions = excludeExtensions.concat(this.sitecrawlerOptions.exclude);
    }

    // format extensions
    exts = excludeExtensions.join('|');

    // regular exprssion extensions
    regex = new RegExp('\.(' + exts + ')', 'i');
    ...
    if (this.sitecrawlerOptions.stripQuerystring) {
...
/**
 * Converts local sitemap.xml to JSON format
 * @author Steven Britton
 * @date   2016-04-25
...
Sitemap.prototype.create = function() {
    ...
    */
    ...
    this.crawler.on('fetch404', function(item) {
        ...
        error.collector404.push(item.url);
    });
...
Sitemap.prototype.writeCollections = function(collectionName, collection, callback) {
